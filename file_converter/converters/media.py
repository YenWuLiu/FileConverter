...
def _run_ffmpeg(args: list[str]) -> None:
    ...
    if r.returncode != 0:
        raise ConvertError(f"ffmpeg 转换失败: {(r.stderr or '').strip()[-300:]}")
...
def video_convert(src: Path, dst: Path, **opts):
    ...
    _run_ffmpeg([_ffmpeg_exe(), "-nostdin", "-i", str(src), str(dst)])
...
def video_to_audio(src: Path, dst: Path, **opts):
    _run_ffmpeg([_ffmpeg_exe(), "-nostdin", "-i", str(src), "-vn", str(dst)])
